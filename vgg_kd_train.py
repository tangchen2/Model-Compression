...
student_weight = "STUDENT_PATH"
teacher_weight = "TEACHER_PATH"
epochs = 200
best_acc = 0.
...
print("================= temperature: {}, alpha: {}, lr: {} ========================".format(temperature, alpha, args.lr))
##########################
# data set
##########################
...
student = VGG(depth=16, dataset='cifar10', slim_channel="quarter")
...
#        print("=========")
#        print("student_output {}".format(F.softmax(student_output[0])))
#        print("student temperature {}".format(F.softmax(student_output[0]/temperature)))
#        print("hard loss {}, soft loss {}".format(hard_loss, soft_loss))
...
#        correct += student_predict.eq(batch_label.data.view_as(student_predict)).cpu().sum()
...
def test(epoch):
    ...
    with torch.no_grad():
        for batch_data, batch_label in test_loader:
            ...
            student_output = student(batch_data)
            _, student_predict = student_output.max(1)
#            print("student predict shape {}, batch label shape {}".format(student_predict.shape, batch_label.shape))
            correct += student_predict.eq(batch_label).sum().item()
#            correct += student_predict.eq(batch_label.data.view_as(student_predict)).cpu().sum()
            total += batch_data.size(0)
